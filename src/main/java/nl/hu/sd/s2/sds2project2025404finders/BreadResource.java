...
public class BreadResource {
    ...
    public Response processOrder(List<Map<String, Object>> orderItems) {
        ...
        // Now deduct the stock for each item
        for (Map<String, Object> item : orderItems) {
            // Get bread ID and quantity (same conversion as above)
            Object idObj = item.get("id");
            int breadId;
            if (idObj instanceof Integer) {
                breadId = (Integer) idObj;
            ...
            // Deduct the stock (we already checked it exists and has enough stock)
            boolean success = BreadDAO.deductStock(breadId, quantity);
            
            // This should never fail since we checked above, but just in case
            if (!success) {
        ...
        // All stock has been successfully deducted
        return Response.ok(Map.of("success", true, "message", "Order processed successfully"))
            .build();
    }
