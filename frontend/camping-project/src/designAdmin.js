...
const setControlsEnabled = enabled => {
    [themeSelector, applyBtn, resetBtn, deleteBtn, saveCustomThemeBtn]
        .filter(Boolean)
...
const saveCustomTheme = async () => {
    ...
    const navbar = $("customNavbarBg").value; // 👈 get the navbar color

    const newTheme = {
        "--bg": bg,
        "--text": getContrastColor(bg),
        "--card-bg": card,
        "--navbar-bg": navbar, // 👈 fixed here
        "--accent": accent,
        "--muted": getContrastColor(card) === "#000000" ? "#6b7280" : "#d1d5db"
    };
...
const deleteTheme = async () => {
    ...
    try {
        const res = await fetch(`${BASE_URL}/${name}`, { method: "DELETE" });
        if (!res.ok) {
            const err = await res.json();
            showToast(err.error || "Er is een fout opgetreden bij opslaan");
            return;
