...
class RegistrationResourceTest {
    ...
    void setUp() {
        ...
        // ensure that admin always exists
    ...
    void registrationSucceedsWithValidData() {
        ...
        assertEquals("Registratie gelukt.", body.get("message"));
        assertNotNull(body.get("id"));

        // also check that its actually in the repository
    ...
    void registrationFailsWhenEmailAlreadyExists() {
        // admin@camping.nl already exists
        Registration reg = new Registration();
        reg.setName("Dup");
        reg.setSurname("User");
        reg.setEmail("admin@camping.nl");
        reg.setPassword("Whatever123!");
        ...
        assertEquals("E-mailadres is al geregistreerd.", body.get("message"));
    }
    ...
    void registrationFailsWhenRequiredFieldsMissing() {
        ...
        assertNotNull(body);
        assertEquals(false, body.get("ok"));
        assertEquals("Vul alle verplichte velden in.", body.get("message"));
    }
}
